...
struct MLP {
    ...
    float GAMMA;        /* coeficiente de Weight Decay */
...
int arquitec(char *filename,struct MLP *nn,struct DATOS *datos){
  ...
  printf("\nMomentum: %f",nn->u);
  printf("\nCoeficiente de Weight Decay: %.10f",nn->GAMMA);
...
int train(struct MLP *nn,struct DATOS *datos){
  ...
  for (iter=1; iter<=nn->ITER; iter++) {
    ...
    /* constante de weight decay (Mitchell, pag 117) */
    /* debo calcularla cada vez por si se modifica eta durante el algoritmo */
    float weight_decay = 1 - 2 * nn->GAMMA * eta;
    ...
    nn->w2[i][j] *= weight_decay;  /*correccion: weight decay*/
		nn->w2[i][j] += nn->dw2[i][j]; /*correccion: descenso por el gradiente*/
	  }
    ...
    nn->w1[i][j] *= weight_decay;  /*correccion: weight decay*/
		nn->w1[j][k] += nn->dw1[j][k]; /*correccion: descenso por el gradiente*/
	  }
    ...
    if ((iter % nn->NERROR) == 0) {
      ...
      if(datos->PR==datos->PTOT){
          /* siempre deberia entrar a este if con los .net del apartado c*/
          /* el termino de penalizacion es mse + gamma * sumatoria del cuadrado de los pesos */
          /* Mitchell pagina 117 */
          /* uso mse_valid para almacenarlo porque segun el enunciado es lo que queremos ignorar */

          /* arranco en 0 */
          mse_valid = 0;

          /* sumo los pesos al cuadrado de las aristas entre capas 1 y 2 */
          for(i = 0; i <= nn->N1; i++)
      			for(j = 1; j <= nn->N2; j++)
				        mse_valid += nn->w1[j][i] * nn->w1[j][i];

          /* sumo los pesos al cuadrado de las aristas entre capas 2 y 3 */
		      for(i = 0; i <= nn->N2; i++)
			       for(j = 1; j <= nn->N3; j++)
				         mse_valid += nn->w2[j][i] * nn->w2[j][i];

          /* multiplico a la sumatoria por gamma y le sumo el mse */
          mse_valid = mse_train + nn->GAMMA * mse_valid;

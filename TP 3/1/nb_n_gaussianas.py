...
class NaiveBayes:
    def __init__(self, input: InputData):
        ...
        for klass in range(self.classCount):
            for attribute in range(input.inputCount):
                print(
                    f"Clase {klass}. Atributo {attribute}. Media: {self.distribution[klass][attribute].mean}. Varianza: {self.distribution[klass][attribute].variance}"
                )
...
def main():
    if len(sys.argv) != 2:
        print("Modo de uso: nb_n_gaussianas.py <filename>")

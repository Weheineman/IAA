...
# Read from file.
inputFile = open(f"NB{fileStem}MedianError.txt", "r")
trainErrorList = []
validErrorList = []
testErrorList = []
binCountList = []
...
plt.title(f"NB using Median for dataset: {fileStem}")
plt.xticks(x[start::step], x[start::step])
plt.xlabel("Amount of bins")
plt.ylabel("Percentage error")
plt.legend()
# plt.savefig(fname=f"SmallNB{fileStem}MedianGraph")
plt.savefig(fname=f"NB{fileStem}MedianGraph")

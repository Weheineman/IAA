  ...
  *discrete_error=*mse=0.0;
  ...
  for (patron=pat_ini; patron<pat_fin; patron++) {
   ...
   /*nu tiene el numero del patron que se va a presentar*/
    ...
    /*la neurona de salida con valor maximo es la clase predicha*/
    int predictedIndex = 1;
    float maxOutValue = nn->x3[1];
    for(i = 2; i <= nn->N3; i++)
      if(maxOutValue < nn->x3[i]){
        maxOutValue = nn->x3[i];
        predictedIndex = i;
      }

    /*si el vector de entrada no tiene 0.9 en el indice predicho
      es un error de clasificacion. me da miedo comparar floats
      asi que pongo 0.8 por si acaso.                           */
    *discrete_error += (S[nu][nn->N1 + predictedIndex] < 0.8);

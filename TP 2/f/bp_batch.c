...
struct MLP {
    ...
    int BATCHSIZE;      /* Tamanio del batch de entrenamiento */
...
int arquitec(char *filename,struct MLP *nn,struct DATOS *datos){
  ...
  /* Tamanio del batch de entrenamiento */
  fscanf(b,"%d",&nn->BATCHSIZE);
  error=(nn->BATCHSIZE > datos->PR);
  if(error){
    printf("Error: el batch no puede ser mayor a la cantidad de patrones de entrenamiento.\n");
    return 1;
  }
  ...
  printf("\nTamanio del batch: %d",nn->BATCHSIZE);
  printf("\nEpocas: %d",nn->ITER);
...
int train(struct MLP *nn,struct DATOS *datos){
  ...
  /* acumuladores de los gradientes dentro de un batch */
  float grad2Acum[nn->N2 + 1], grad3Acum[nn->N3 + 1];
  ...
  for (iter=1; iter<=nn->ITER; iter++) {
    ...
    for(nu1=0;nu1<datos->PR;nu1++) {
      ...
      /*resetear los acumuladores a cero*/
      for(i = 1; i <= nn->N2; ++i)
        grad2Acum[i] = 0;
      ...
      /*iterar dentro de un batch o hasta que no haya mas patrones de entrenamiento*/
      for(nuBatch = 0; nuBatch < nn->BATCHSIZE && nu1 < datos-> PR; nuBatch++, nu1++) {
